...
public:
	...
	template <class T>
	static void read_binary_value(std::ifstream& infile, T& output)
	{
		infile.read(reinterpret_cast<char*>(&output), sizeof(T));
	}

	static void load_graph_file_streaming(const std::string& filename, Graph& vecGraph)
	{
		static_assert(sizeof(Edge) == sizeof(uint32_t) + sizeof(float),
		...
		size_t numNodes = 0;
		read_binary_value(infile, numNodes);
		if (numNodes > std::numeric_limits<uint32_t>::max())
		{
			std::cout << "The graph has too many nodes for OPIM uint32_t node ids: " << numNodes << '\n';
			exit(1);
		}
		vecGraph.clear();
		vecGraph.resize(numNodes);
		for (size_t node = 0; node < numNodes; ++node)
		{
			size_t degree = 0;
			read_binary_value(infile, degree);
			vecGraph[node].resize(degree);
			if (degree > 0)
			{
				infile.read(reinterpret_cast<char*>(vecGraph[node].data()),
				            static_cast<std::streamsize>(sizeof(Edge) * degree));
			}
		}
		if (!infile)
		{
			std::cout << "Failed while reading graph file: " + filename << '\n';
			exit(1);
		}
	}
	...
	{
		if (file_exists(graphName)
			&& (ends_with(graphName, ".bin")
				|| ends_with(graphName, ".vec.graph")
				|| ends_with(graphName, ".vec.rvs.graph")))
		{
			load_graph_file_streaming(graphName, vecGraph);
			return;
		}
		std::string postfix = ".vec.graph";
		if (isReverse) postfix = ".vec.rvs.graph";
		const std::string filename = graphName + postfix;
		load_graph_file_streaming(filename, vecGraph);
	}
	...
	static void write_result(const std::string& outFileName, const TResult& resultObj, const std::string& outFolder)
	{
		...
		std::cout << "  |#RR sets (per graph): " << RRsetsSize << std::endl;
		std::cout << "   --------------------" << std::endl;
		...
		if (outFileNew.is_open())
		{
			...
			outFileNew << "#RR sets (per graph): " << RRsetsSize << std::endl;
			outFileNew.close();
		}
	}
